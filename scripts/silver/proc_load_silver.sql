...
populate the 'silver' schema tables from the 'bronze' schemaI .
Actions Performed:
...
BEGIN
	...
	BEGIN TRY
		...
		PRINT '>>Insertin Data Into Table: silver.crm_cust_info';
		...
		*,
		...
		PRINT '>>Insertin Data Into Table: silver.crm_prd_info';
		...
		CASE WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
			 ...
			 ELSE 'N/A'
		END gen -- Normaliza gender values and handle unknown cases
		...
		PRINT '>>Insertin Data Into Table: silver.erp_loc_a101';
		...
		REPLACE(cid, '-', '') AS cid,
			CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
				 ...
				 ELSE TRIM(cntry)
			END cntry -- Normalize and handle missing or clank country codes
